 ...
 void mailcheck(char uname[])
{
 ...
 int flag=0;
 if (!fi)
 cout<<"\n\nYour Mail History is not available";
 else
 {
  //cout<<"\n\nthe person's personal file is available....";
 ...
 if(flag)
 {
  ...
  cout<<"\n\n Mail no."<<setw(20)<<"From"<<setw(20)<<"Date";
  while(f.read((char*)&Y,sizeof(Y)))
  {
   lines();
   cout<<ch++; cout<<ch++<<setw(20)<<Y.from<<setw(20)<<Y.cal.d<<"."<<Y.cal.m<<"."<<Y.cal.y;
...
info X2;
 ...
 if(flag)
 {
  cout<<"\n\nthe person's name is : "; puts(X2.fname);
  cout<<"\nthe person's user login id is : "<<X2.id;
  ...
  strcat(add,X2.id);
  strcat(add,"contacts.dat");
ifstream fc(add,ios::binary);
contct a;
cout<<"\n\n\n Choose the person to whom you want to send the mail  \n";
int i=0;
